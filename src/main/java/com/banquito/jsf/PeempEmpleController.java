...
import com.banquito.controladores.PeempEmpleFacade;
...
public class PeempEmpleController implements Serializable {
    ...
    @EJB
    private com.banquito.controladores.PeempEmpleFacade ejbFacade;
    private List<PeempEmple> items = null;
    ...
    public PeempEmple prepareCreate() {
        selected = new PeempEmple();
        initializeEmbeddableKey();
        return selected;
    }

    public void create() {
        persist(PersistAction.CREATE, ResourceBundle.getBundle("/Bundle").getString("PeempEmpleCreated"));
    ...
    public void update() {
        persist(PersistAction.UPDATE, ResourceBundle.getBundle("/Bundle").getString("PeempEmpleUpdated"));
    }

    public void destroy() {
        persist(PersistAction.DELETE, ResourceBundle.getBundle("/Bundle").getString("PeempEmpleDeleted"));
        if (!JsfUtil.isValidationFailed()) {
    ...
    public List<PeempEmple> getItems() {
        if (items == null) {
